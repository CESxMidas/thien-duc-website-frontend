...
function SectionEyebrow({ children }: { children: ReactNode }) {
  return (
    <p className="inline-flex h-11 items-center gap-3 bg-[#B06613] px-5 text-sm font-semibold uppercase tracking-[0.24em] text-white shadow-[0_8px_24px_rgba(176,102,19,0.22)]">
      <span className="h-1 w-10 shrink-0 bg-[#fdcd04]" aria-hidden="true" />
      {children}
    </p>
  );
}
...
export function HomeHero() {
  return (
    <section className="relative overflow-hidden bg-[linear-gradient(135deg,#f6f3ee_0%,#fff9ec_45%,#f6f3ee_100%)]">
      <div
        className="pointer-events-none absolute inset-y-0 left-0 w-1 bg-[#fdcd04]"
        aria-hidden="true"
      />
      ...
      <div className="relative mx-auto grid max-w-7xl gap-x-10 gap-y-5 px-6 py-20 lg:grid-cols-[1.1fr_0.9fr] lg:grid-rows-[auto_1fr] lg:items-start">
        ...
        <div className="order-3 lg:col-start-2 lg:row-start-1 lg:flex lg:justify-start">
          <SectionEyebrow>Năng lực trọng tâm</SectionEyebrow>
        </div>

        <div className="order-2 lg:col-start-1 lg:row-start-2">
          <h1 className="max-w-3xl text-4xl font-semibold leading-tight text-[#191919] md:text-6xl">
            {homeHero.title}
          </h1>
          <p className="mt-5 max-w-2xl border-l-4 border-[#fdcd04] bg-white/70 py-1 pl-5 text-lg leading-8 text-[#59646a]">
            {homeHero.description}
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              href={homeHero.primaryCta.href}
              className="inline-flex h-11 items-center bg-[#B06613] px-5 text-sm font-semibold text-white shadow-[0_8px_24px_rgba(176,102,19,0.28)] transition hover:bg-[#7f4b0d]"
            >
              {homeHero.primaryCta.label}
            </Link>
            <Link
              href={homeHero.secondaryCta.href}
              className="inline-flex h-11 items-center border border-[#B06613]/35 bg-white px-5 text-sm font-semibold text-[#7f4b0d] transition hover:border-[#B06613] hover:bg-[#fff8ea]"
            >
        ...
        <div className="order-4 overflow-hidden border border-[#B06613]/15 bg-white shadow-[0_20px_48px_rgba(25,25,25,0.08)] lg:col-start-2 lg:row-start-2">
          <div className="grid gap-0 p-2 sm:p-3">
            {homeStrengths.map((item, index) => {
              ...
              return (
                <div
                  key={item.title}
                  className="group flex gap-4 border-b border-[#f0e6d6] p-4 transition last:border-b-0 hover:bg-[#fffaf0]"
                >
