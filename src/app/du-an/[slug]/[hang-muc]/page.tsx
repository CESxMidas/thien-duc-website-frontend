...
export async function generateMetadata({
  params,
}: ProjectItemPageProps): Promise<Metadata> {
  const { slug, "hang-muc": itemSlug } = await params;
  const item = await getProjectItem(slug, itemSlug);
...
export default async function ProjectItemPage({ params }: ProjectItemPageProps) {
  ...
  const projectHref = `${routes.projects}/${project.slug}`;
